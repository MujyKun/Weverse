...
from Weverse import WeverseClientSync, InvalidToken, BeingRateLimited, models
...
Synchronous Examples for Weverse.
...
start_kwargs = {
    # Will load ALL followed community posts. If following majority communities, this will take more than 10 minutes
    # to complete. If you are not in need of OLDER posts, you can set it to False and it should load within a few
    # seconds.
...
try:
    ...
    weverse_client.start(**start_kwargs)  # start the client process to all needed data.
...
def check_new_notifications() -> Optional[models.Notification]:
    ...
    if not weverse_client.user_notifications:
        return

    # We can now get the latest notification
    latest_notification = weverse_client.user_notifications[0]
...
while continue_loop:
    ...
    new_notification = check_new_notifications()
